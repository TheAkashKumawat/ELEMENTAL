    ...
    <div className="space-y-8">
      ...
      <div className="max-w-2xl mx-auto text-center space-y-4">
        ...
        <h1 className="text-3xl font-serif font-light text-stone-950">
          {query ? `Results for “${query}”` : 'Search Creations'}
        </h1>
        ...
        <form onSubmit={handleSearchSubmit} className="relative flex items-center max-w-xl mx-auto">
          ...
          <input
            ...
            placeholder="Search by name, fabric, category, or color..."
            className="w-full h-12 pl-12 pr-10 bg-stone-50 border border-stone-300 rounded-full text-sm text-stone-900 focus:outline-none focus:border-stone-900 focus:bg-white transition-colors"
          />
          {query && (
            <button
              type="button"
              onClick={() => setQuery('')}
              className="absolute right-4 text-stone-400 hover:text-stone-700"
            >
              <X className="w-4 h-4" />
            </button>
          )}
