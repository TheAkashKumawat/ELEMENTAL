      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-2 pointer-events-none max-w-sm w-full px-4 sm:px-0">
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={clsx(
              'pointer-events-auto flex items-center justify-between p-4 rounded-lg shadow-xl border backdrop-blur-md transition-all animate-slideUp text-sm',
              toast.type === 'success' && 'bg-stone-900 text-stone-100 border-stone-800',
              toast.type === 'error' && 'bg-rose-950 text-rose-100 border-rose-800',
              toast.type === 'info' && 'bg-stone-800 text-stone-100 border-stone-700'
            )}
          >
